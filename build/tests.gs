...
function testSSaaDB() {
    var db = new SSaaDB(PropertiesService.getScriptProperties().getProperty('SpreadSheet_ID'));
    // Test createTable
    ...
    // Test getEntries
    var entries1 = db.getEntries('TestTable1');
    ...
    var updatedEntries1 = db.getEntries('TestTable1', { ID: { operator: '==', value: 1 } });
    if (updatedEntries1[0]['Age'] !== 26) {
        throw new Error('Failed to update entries');
    }
    // Test deleteEntries
    db.deleteEntries('TestTable1', { ID: { operator: '==', value: 1 } });
    var entriesAfterDelete = db.getEntries('TestTable1');
    if (entriesAfterDelete.length !== 1) {
        throw new Error('Failed to delete entries');
    }
    // Test joinTables
    var joinedEntries = db.joinTables('TestTable1', 'TestTable2', 'ID');
